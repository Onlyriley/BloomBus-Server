...
export default function simulateRuns(
  runs: Array<ShuttleRun>,
  ...
  runs.forEach((shuttleRun: ShuttleRun) => {
    ...
    setInterval(() => {
      ...
      console.log(
        `${i} - ${shuttlePoint.Duration}, ${shuttlePoint["Latitude(WGS84)"]} ${
          shuttlePoint["Longitude(WGS84)"]
        }`
      );
      const geoJSON = {
        ...
        properties: {
          ...
          bearing: turf.bearing(
            turf.point(prevPointCoords),
            turf.point(shuttlePointCoords)
          ),
